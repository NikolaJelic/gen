...
void Gene::set_phenotypes() {
  phenotype_x = calculate_phenotype(chromosome_to_decimal(chromosome_x), min_x, max_x);
  phenotype_y = calculate_phenotype(chromosome_to_decimal(chromosome_y), min_x, max_x);
}

[[nodiscard]] float Gene::calculate_phenotype(float value, float min, float max) const {
  float in_min = 0.0f;
